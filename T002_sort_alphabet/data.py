...
from generateWordTargets import genInTargetBatch
from wordFeatures import featurify
import config
...
def batch(batch_size, min_len=5, max_len=12):
  array_len = torch.randint(low=min_len, 
                            high=max_len + 1,
                            size=(1,))
  inputs, targets = genInTargetBatch(batch_size)

  inputsFeatures = featurify(inputs)

  inputsFeaturesTensor = torch.LongTensor(inputsFeatures)
  targets = torch.LongTensor(targets)

  #x = torch.randint(high=1000, size=(batch_size, array_len), device=config.DEVICE)

  return inputsFeaturesTensor, targets
