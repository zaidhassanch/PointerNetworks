...
class PointerNetwork(nn.Module):
  ...
  def forward(self, 
    ...
    outputs = torch.zeros(out.size(1), out.size(0), dtype=torch.long, device=config.DEVICE)
    
    # First decoder input is always 0
    # dec_in: (BATCH, 1, 1)
    dec_in = torch.zeros(out.size(0), 1, 1, dtype=torch.float, device=config.DEVICE)
