...
def train(model, device, load_model, save_model, german_vocab, english_vocab, train_data, valid_data,
                                                                test_data, batch_size,max_length=50):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_iterator):
            ...
            outputs = [english_vocab.stoi["<sos>"]] * 32
            # outputs = outputs.reshape(-1, 32, len(german_vocab))
            output_save = torch.cuda.FloatTensor()
            for i in range(trg.shape[0]):
                trg_tensor = torch.LongTensor(outputs).unsqueeze(0).to(device)

                output = model(inp_data, trg_tensor)

                output_save = torch.cat((output_save, output), 0)

                best_guess1 = output.argmax(2)
                #best_guess = best_guess1[-1, :]
                trg_tensor = torch.cat((trg_tensor, best_guess1), 0)
            ...
            output_save = output_save.reshape(-1, output_save.shape[2])
            target = target[1:].reshape(-1)

            optimizer.zero_grad()

            loss = criterion(output_save, target)
