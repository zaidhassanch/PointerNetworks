...
def printSample(text, trn, example_id):
	print(text)
	ex = trn.examples[example_id].orig
	dec_ex = sp_gec.decode(ex)
	print(ex)
	print(dec_ex)

	print("Correction1:")
	ex = trn.examples[example_id].correction1
	dec_ex = sp_gec.decode(ex)
	print(ex)
	print(dec_ex)
...
trn, tst = TabularDataset.splits(path = ".", train = "test10k.tsv", test = "test10k.tsv", format='tsv', skip_header=False, fields = tv_datafields)
...
for idx, batch in enumerate(train_iterator):
	print(batch.orig[0])
	v = sp_gec.decode(batch.orig[0].tolist())
	print(v)
	print(batch.orig[1])
	v = sp_gec.decode(batch.orig[1].tolist())
	print(v)
