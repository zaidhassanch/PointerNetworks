...
def getData2(ger_tok, eng_tok, from_TSV = False):
  if from_TSV == False:
    ...
    eng_voc = build_vocab('.data/multi30k/train.de', eng_tok) #vocab from training data

    train_data = data_process('.data/multi30k/train.en', '.data/multi30k/train.de', ger_voc, eng_voc, ger_tok, eng_tok)
    val_data = data_process('.data/multi30k/val.en', '.data/multi30k/val.de', ger_voc, eng_voc, ger_tok, eng_tok)
    test_data = data_process('.data/multi30k/test2016.en', '.data/multi30k/test2016.de', ger_voc, eng_voc, ger_tok, eng_tok)
  else:
    ger_voc = build_vocab_tsv('.data/multi30k/train.tsv', ger_tok, column=0) #vocab from training data
    eng_voc = build_vocab_tsv('.data/multi30k/train.tsv', eng_tok, column=1)

    train_data = data_process_tsv('.data/multi30k/train.tsv', ger_voc, eng_voc, ger_tok, eng_tok)
    #val_data = data_process_tsv('.data/multi30k/val.tsv', ger_voc, eng_voc, ger_tok, eng_tok)
    test_data = data_process_tsv('.data/multi30k/test2016.tsv', ger_voc, eng_voc, ger_tok, eng_tok)
