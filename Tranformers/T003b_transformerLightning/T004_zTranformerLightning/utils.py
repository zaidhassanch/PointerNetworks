...
def translate_sentence_tokens_bpe(model, tokens, german_vocab, english_vocab, device, max_length=50):
...
def computeBLEU_bpe(data, model, german, english, device):
    ...
    for example in tqdm(data):
        ...
        prediction = translate_sentence_tokens_bpe(model, src, german, english, device)
        prediction = prediction[:-1]  # remove <eos> token
